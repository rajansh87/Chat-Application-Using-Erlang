...
receives(UserName) ->
  receive
    ...
    {ServerName, MsgHistory,latest_n_history} ->
      ...
      Len = lenList(DisplayList),
      if Len < N ->
        lists:foreach(fun({Time,From,Message}) ->
          io:format("~p ~p: ~p~n",[Time,From,Message])
                      end,
          DisplayList);
        true ->
          LastNMsgs = lists:nthtail(Len-N,DisplayList),
          lists:foreach(fun({Time,From,Message}) ->
            io:format("~p ~p: ~p~n",[Time,From,Message]) end, LastNMsgs)
      end,
      receives(UserName);
    ...
    {ServerName, MsgHistory,offline_message_history} ->
      ...
      DisplayList = lists:reverse(MsgHistory),
      lists:foreach(fun({Time,From,Message}) ->
            io:format("~p ~p: ~p~n",[Time,From,Message]) end, DisplayList),
